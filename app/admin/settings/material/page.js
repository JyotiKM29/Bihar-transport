...
import ProductList from "../../booking/ProductAdd";
...
const AddUnit = () => {
  ...
  useEffect(() => {
    const fetchUnits = async () => {
      try {
        ...
        const response = await fetch(`/api/getProduct/${userId}`);
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const response = await fetch("/api/setting/paymentTerm/create", {
        method: "POST",
        headers: {
  ...
  return (
    <div className="min-h-full w-full rounded-3xl bg-white px-6 py-4 shadow-sm">
      ...
      >
         ...
         <ProductList />
        <p>(Click on this + icon to add new mateiral)</p>
      </form>
