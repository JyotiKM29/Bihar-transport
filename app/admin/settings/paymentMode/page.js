...
const AddUnit = () => {
  ...
  useEffect(() => {
    const fetchUnits = async () => {
      try {
        ...
        const response = await fetch(`/api/setting/paymentMode/get/${userId}`);
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const response = await fetch("/api/setting/paymentMode/create", {
        method: "POST",
        headers: {
  ...
  return (
    <div className="min-h-full w-full rounded-3xl bg-white px-6 py-4 shadow-sm">
      <h2 className="font-semiBold mt-12 text-3xl lg:mt-0">Add a New Payment Mode:</h2>
      ...
      >
        ...
        <Button type="submit">{loading ? "Adding..." : "Add Payment Mode"}</Button>
      </form>
